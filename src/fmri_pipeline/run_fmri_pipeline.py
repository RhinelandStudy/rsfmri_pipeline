...
from .fmri_pipeline import create_preprocess_wf
from .fmri_pipeline import create_preprocess_wf_nocln
...
def get_pp_wf_nocln(scans_dir, work_dir,
                         outputdir, subject_ids,
                         traindata,fixth,mthreads,
                         spm, mcr, wfname='preprocess_workflow_xica'):

    ppwf = pe.Workflow(name=wfname)

    ppwf = create_preprocess_wf_nocln(scans_dir=scans_dir,
                              work_dir=work_dir,
                              subj_ids=subject_ids,
                              traindata=traindata,
                              fixth=fixth,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run fMRI preprocess pipeline for RestingState '\
                                     ...
                                     ' ~/data/output -w ~/data/work -p [NProc] -t [NThreads] '\
                                     '\n\n'
    ...
    group.add_argument('--no_ica',help='Do not use ica fix cleaned data.',
                        action='store_true', default=False)

    group.add_argument('-d', '--traindata', help='fix train weights data',required=False) 
    parser.add_argument('--fixth',help='fix classifier threshold',default=20,type=int)
    parser.add_argument('-b', '--debug', help='debug mode', action='store_true')
    parser.add_argument('-p', '--processes', help='parallel processes', \
                        default=1, type=int)
    parser.add_argument('-m', '--mthreads', help='matlab node threads', default=2,\
                        type=int)
    ...
    if args.no_ica is False and args.traindata is None:
        print("\n rs_fmri_pipeline:\n error: must provide --nocln or -d option.\n")
        sys.exit(1)
    ...
    if args.no_ica is True:
        preproc_wf = get_pp_wf_nocln(scans_dir=scans_dir,
                           work_dir=work_dir,
                           outputdir=out_dir,
