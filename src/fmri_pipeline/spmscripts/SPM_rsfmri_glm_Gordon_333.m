...
function SPM_rsfmri_glm_Gordon_333(swurestfile,rprestfile,wc2file,wc3file)
...
opts = detectImportOptions(strcat(dir_parcellations, fs, 'Gordon_2016_Parcels.xlsx'));
t1 = readtable(strcat(dir_parcellations, fs, 'Gordon_2016_Parcels.xlsx'), opts);
...
for roi = 1:size(t1,1)
    ...
    if roi < 10
        matlabbatch{roi}.spm.util.voi.name = ['Gordon_', char(t1.Community(roi)), '_', char(t1.Hem(roi)), '_00', num2str(t1.ParcelID(roi))];
    elseif roi < 99
        matlabbatch{roi}.spm.util.voi.name = ['Gordon_', char(t1.Community(roi)), '_', char(t1.Hem(roi)), '_0', num2str(t1.ParcelID(roi))];
    else
        matlabbatch{roi}.spm.util.voi.name = ['Gordon_', char(t1.Community(roi)), '_', char(t1.Hem(roi)), '_', num2str(t1.ParcelID(roi))];
    end
    matlabbatch{roi}.spm.util.voi.roi{1}.label.image = cellstr(spm_select('ExtFPList', dir_parcellations, '^rGordon_Parcels_MNI_222.*\.nii$'));
    matlabbatch{roi}.spm.util.voi.roi{1}.label.list = t1.ParcelID(roi);
...
nVOI = dir(strcat(SubjFFX2, fs, 'VOI_Gordon*.mat'));

if size(nVOI,1) < 333

    for ii = 1:size(nVOI,1)
        clear tmp1;
        tmp1 = regexp(nVOI(ii).name, {'_'}, 'split');
        tmp2(ii,1) = str2double(char(tmp1{1}(:,5)));
    end

    tmp2 = find(~ismember(t1.ParcelID, tmp2));

    for jj = 1:size(tmp2,1)

        clear ttt centroid_mni;
        ttt = regexp(t1.Centroid_MNI_(tmp2(jj,1)), ' ', 'split');
        centroid_mni = str2double(ttt{1}(1,:));
        ...
        if tmp2(jj,1) < 10
            matlabbatch{jj}.spm.util.voi.name = ['Gordon_', char(t1.Community(tmp2(jj,1))), '_', char(t1.Hem(tmp2(jj,1))), '_00', num2str(t1.ParcelID(tmp2(jj,1)))];
        elseif tmp2(jj,1) < 100
            matlabbatch{jj}.spm.util.voi.name = ['Gordon_', char(t1.Community(tmp2(jj,1))), '_', char(t1.Hem(tmp2(jj,1))), '_0', num2str(t1.ParcelID(tmp2(jj,1)))];
        else
            matlabbatch{jj}.spm.util.voi.name = ['Gordon_', char(t1.Community(tmp2(jj,1))), '_', char(t1.Hem(tmp2(jj,1))), '_', num2str(t1.ParcelID(tmp2(jj,1)))];
        end
        matlabbatch{jj}.spm.util.voi.roi{1}.sphere.centre     = centroid_mni;
...
clear nVOI varnames FC_corr tmp1 tmp2 tmp3 tmp4 allnetworks;
...
for ii = 1:size(nVOI,1)
    ...
    tmp4(ii,:) = [ii tmp3{1}(:,[3:5])];
end

[G_network TID] = findgroups(tmp4(:,2));
...
for gg = 1:size(TID,1)
    clear int_network;
    int_network = allnetworks(find(cell2mat(allnetworks(:,5))==gg),:);
    
    for ii = 1:size(int_network,1)
        varnames{1,count} = nVOI(cell2mat(int_network(ii,1))).name(12:end-6);
        clear xY;
...
clear t2;
t2 = array2table(FC_corr, 'VariableNames', varnames);
writetable(t2, strcat(SubjFFX2, fs, 'FC_Gordon_333p_simple.csv'));
