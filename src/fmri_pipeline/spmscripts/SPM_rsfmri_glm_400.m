...
function SPM_rsfmri_glm_400(swurestfile,rprestfile,wc2file,wc3file)
...
opts = detectImportOptions(strcat(dir_parcellations, fs, 'Schaefer2018_400Parcels_17Networks_order_FSLMNI152_2mm.Centroid_RAS.csv'));
t1 = readtable(strcat(dir_parcellations, fs, 'Schaefer2018_400Parcels_17Networks_order_FSLMNI152_2mm.Centroid_RAS.csv'), opts);
...
for roi = 1:size(t1,1)
    ...
    matlabbatch{roi}.spm.util.voi.roi{1}.label.image = cellstr(spm_select('ExtFPList', dir_parcellations, '^rSchaefer2018_400Parcels_17Networks.*\.nii$'));
...
writetable(t2, strcat(SubjFFX2, fs, 'FC_Schaefer_17Networks_400p_simple.csv'));
