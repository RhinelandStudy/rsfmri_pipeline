...
function SPM_rsfmri_glm_200_17_7networks(swurestfile,rprestfile,wc2file,wc3file)

% Date created: 2021-03-12, Author: Hweeling Lee; last updated: 2021-12-03
...
% current output of the GLM is a 200 ROI functional connectivity matrix. If research question is intereested in a specific ROI, a 3rd GLM should be created with the signal intensity of that ROI.
% output to be saved: FC_Schaefer_17Networks_200p_fullcorr.csv,
% FC_Schaefer_7Networks_200p_fullcorr.csv,
% FC_Schaefer_17Networks_200p_raw.csv, FC_Schaefer_7Networks_200p_raw.csv
...
opts = detectImportOptions(strcat(dir_parcellations, fs, 'Schaefer2018_200Parcels_7Networks_order_FSLMNI152_2mm.Centroid_RAS.csv'));
t2 = readtable(strcat(dir_parcellations, fs, 'Schaefer2018_200Parcels_7Networks_order_FSLMNI152_2mm.Centroid_RAS.csv'), opts);
...
clear t3;
t3 = array2table(tmp_raw, 'VariableNames', varnames);
writetable(t3, strcat(SubjFFX2, fs, 'FC_Schaefer_17Networks_200p_raw.csv'));
clear t4;
t4 = array2table(FC_corr, 'VariableNames', varnames);
writetable(t4, strcat(SubjFFX2, fs, 'FC_Schaefer_17Networks_200p_fullcorr.csv'));

%% for 200 parcels: 7 networks
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% signal extraction for each ROI %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for roi = 1:size(t2,1)
    ...
    matlabbatch{roi}.spm.util.voi.roi{1}.label.image = cellstr(spm_select('ExtFPList', dir_parcellations, '^rSchaefer2018_200Parcels_7Networks.*\.nii$'));
    matlabbatch{roi}.spm.util.voi.roi{1}.label.list = roi;
    matlabbatch{roi}.spm.util.voi.roi{2}.mask.image = cellstr(spm_select('FPList', SubjFFX2, '^mask.*\.nii$'));
    matlabbatch{roi}.spm.util.voi.expression = 'i1 & i2';
...
nVOI = dir(strcat(SubjFFX2, fs, 'VOI_7Networks*.mat'));
...
clear t3;
t3 = array2table(tmp_raw1, 'VariableNames', varnames);
writetable(t3, strcat(SubjFFX2, fs, 'FC_Schaefer_7Networks_200p_raw.csv'));
clear t4;
t4 = array2table(FC_corr, 'VariableNames', varnames);
writetable(t4, strcat(SubjFFX2, fs, 'FC_Schaefer_7Networks_200p_fullcorr.csv'));
