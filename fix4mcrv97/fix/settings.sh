...
if [ "$FSL_FIX_MATLAB_MODE" -eq 0 ]; then
	...
	if [ -z "${FSL_FIX_MCRROOT}" ]; then
                if [[ "`hostname -s`" = bn-wl-rs-01 ]];then
                     FSL_FIX_MCRROOT="/opt/MCR2019b"
                else
                     FSL_FIX_MCRROOT="/opt/MCR2019b"
                fi
		#FSL_FIX_MCRROOT="/home/shahidm/software/fix2compile/MATLAB/MATLAB_Compiler_Runtime"
...
if [ -z "${FSL_FIX_CIFTIRW}" ]; then
	FSL_FIX_CIFTIRW='/home/shahidm/software/fix2compile/matlab_ciftireadwrite/';
fi
if [ -z "${FSL_FIX_WBC}" ]; then
	...
	if [ -z "${FSL_FIX_WBC}" ]; then
		...
		WBENCH="/home/shahidm/software/fix2compile/workbench/"
